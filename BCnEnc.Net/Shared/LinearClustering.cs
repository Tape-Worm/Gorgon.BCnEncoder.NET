	...
	internal static class LinearClustering
	{
		...
		private static void InitialClusterCenters(int width, int height, Span<ClusterCenter> clusterCenters, float s, Span<LabXy> labXys)
		{
			switch (clusterCenters.Length)
			{
				case 2:
					{
						int x0 = (int)Math.Floor(width * 0.333f);
						int y0 = (int)Math.Floor(height * 0.333f);

						int x1 = (int)Math.Floor(width * 0.666f);
						int y1 = (int)Math.Floor(height * 0.666f);
				...
				case 3:
					{
						...
						int x1 = (int)Math.Floor(width * 0.666f);
						int y1 = (int)Math.Floor(height * 0.333f);
						int i1 = x1 + y1 * width;
						clusterCenters[1] = new ClusterCenter(labXys[i1]);

						int x2 = (int)Math.Floor(width * 0.5f);
						int y2 = (int)Math.Floor(height * 0.666f);
						int i2 = x2 + y2 * width;
