...
namespace BCnEncoder.Shared
{
	internal static class PcaVectors
	{
		...
		internal static void CalculatePrincipalAxis(ref Matrix4x4 covarianceMatrix, out Vector4 result) {
			...
			for (int i = 0; i < 30; i++) {
				Vector4 dA = Vector4.Transform(result, covarianceMatrix);
		...
		public static void GetExtremePoints(Span<int> colors, Vector3 mean, Vector3 principalAxis, out ColorRgb24 min, out ColorRgb24 max)
		{
			...
			Vector3 minAxis = Vector3.Multiply(principalAxis, minD);
			Vector3 maxAxis = Vector3.Multiply(principalAxis, maxD);
			Vector3 minVec = Vector3.Add(mean, minAxis);
			Vector3 maxVec = Vector3.Add(mean, maxAxis);
		...
		public static void GetExtremePointsWithAlpha(Span<GorgonColor> colors, Vector4 mean, Vector4 principalAxis, out Vector4 min, out Vector4 max)
		{
			...
			{				
				...
				Vector4 v = Vector4.Subtract(colorVec, mean);
			...
			Vector4 minAxis = Vector4.Multiply(principalAxis, minD);
			Vector4 maxAxis = Vector4.Multiply(principalAxis, maxD);
			...
			float rWeight = 0.3f, float gWeight = 0.6f, float bWeight = 0.1f)
		{
			...
			double CalculateError()
			{
				...
				Vector3 diff = Vector3.Subtract(ep1, ep0);
				Vector3 ep3rds = Vector3.Divide(diff, 3.0f);
				Vector3 ep2_3rds = Vector3.Divide(diff, 2.0f / 3.0f);
				Vector3 ep2 = Vector3.Add(ep0, ep3rds);
				Vector3 ep3 = Vector3.Add(ep0, ep2_3rds);
			...
			while (increment > 1 || foundBetter)
			{
				...
				{ // decrement ep0
					...
					Vector3 axisP = Vector3.Multiply(principalAxis, increment * 2);
		...
		{
			Vector3 v = Vector3.Subtract(point, linePoint);
